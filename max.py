...
def maxPop(total_df):
    ...
    fig, ax = plt.subplots()
    ax.bar(avg_population_head['자치구명'], avg_population_head['총생활인구수'], fontproperties=font_name)
    ...
    fig, ax = plt.subplots()
    ax.bar(avg_population_tail['자치구명'], avg_population_tail['총생활인구수'], fontproperties=font_name)
    ax.set_ylabel("평균 생활인구수")
    ax.set_xlabel("자치구")
