...
def maxPop(total_df):
    ...
    st.subheader(f'{acc_year}년 {selected_month} {selected_time} 평균 생활인구수가 가장 많은 자치구')
    ...
    st.subheader(f'{acc_year}년 {selected_month} {selected_time} 평균 생활인구수가 가장 적은 자치구')
    #ax.set_ylabel("평균 생활인구수")
    #ax.set_xlabel("자치구")
...
# 데이터 프레임 로드 예제 (실제 데이터 경로로 변경해야 함, fontproperties=fontprop)
